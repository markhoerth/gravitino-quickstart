...
async def main():
    ...
    print("Gravitino + Trino + MetricFlow MCP App")
    ...
    async with streamablehttp_client("http://127.0.0.1:8001/mcp/", headers=headers) as (gr, gw, _):
        async with streamablehttp_client("http://127.0.0.1:8002/mcp", headers=headers) as (tr, tw, _):
            async with streamablehttp_client("http://127.0.0.1:8003/mcp", headers=headers) as (mr, mw, _):
                async with ClientSession(gr, gw) as gravitino:
                    async with ClientSession(tr, tw) as trino:
                        async with ClientSession(mr, mw) as metricflow:
                            ...
                            t_tools = await trino.list_tools()
                            ...
                            print(f"Ready with {len(g_tools.tools)} Gravitino + {len(t_tools.tools)} Trino + {len(m_tools.tools)} MetricFlow tools\n")
                                    ...
                                    if response.stop_reason == "tool_use":
                                        ...
                                        for block in response.content:
                                            if block.type == "tool_use":
                                                ...
                                                elif tool_name.startswith("trino_"):
